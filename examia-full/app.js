...
const SESSION_STORAGE = "examia_session";
...
async function freeExtract(base64, session) {
  const res = await fetch("/api/extract", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ base64, session }) });
  if (res.status === 429) throw new FreeExhausted();
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Error en el servidor");
  return data.text;
}
async function freeGenerate(textContent, numQ, session) {
  const res = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ textContent, numQ, session }) });
  if (res.status === 429) throw new FreeExhausted();
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Error en el servidor");
  return data;
...
async function startCheckout(session) {
  try {
    const r = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ session }) });
...
async function requestLogin(email) {
  const r = await fetch("/api/login-request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) });
  const d = await r.json();
  if (!r.ok) throw new Error(d.error || "No se pudo enviar el correo");
  return true;
}
async function verifyLogin(token) {
  const r = await fetch("/api/login-verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token }) });
  const d = await r.json();
  if (!r.ok) throw new Error(d.error || "Enlace inválido");
  return d;
}

// ── Components ──
function Header({ view, fallos, email, subscribed, onLogo, onPanel, onFallos, onApiKey, onLogin, onLogout }) {
  return (
    <div className="header">
      ...
      <div className="header-right">
        ...
        {email
          ? <>
              <span className="header-meta" title={email}>{subscribed ? "✨ " : "👤 "}{email.length > 18 ? email.slice(0, 16) + "…" : email}</span>
              <button className="nav-pill" onClick={onLogout}>Salir</button>
            </>
          : <button className="nav-pill" onClick={onLogin}>Iniciar sesión</button>}
        {view === "landing" && <button className="panel-btn" onClick={onPanel}>Empezar</button>}
      </div>
    </div>
  );
}
...
function LoginModal({ onClose, onSent }) {
  ...
  const submit = async () => {
    ...
    try { await requestLogin(email.trim().toLowerCase()); setSent(true); if (onSent) onSent(); }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        {sent ? (
          <>
            <div style={{ fontSize: 32, marginBottom: 16 }}>📬</div>
            <div className="modal-title">Revisa tu correo</div>
            <div className="modal-sub">Te hemos enviado un enlace de acceso a <b>{email}</b>. Ábrelo desde este dispositivo para iniciar sesión. (Mira también spam.)</div>
            <button className="btn btn-ghost btn-lg" style={{ width: "100%" }} onClick={onClose}>Entendido</button>
          </>
        ) : (
          <>
            <div style={{ fontSize: 32, marginBottom: 16 }}>🔓</div>
            <div className="modal-title">Inicia sesión</div>
            <div className="modal-sub">Te enviaremos un enlace mágico por correo. Sin contraseñas. Tu cuenta guarda tu suscripción en cualquier dispositivo.</div>
            <input className="modal-input" type="email" placeholder="tu@email.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} autoFocus />
            {err && <div style={{ color: "var(--red)", fontSize: 13, marginBottom: 12 }}>{err}</div>}
            <button className="btn btn-gold btn-lg" style={{ width: "100%", marginTop: 8 }} onClick={submit} disabled={sending}>{sending ? "Enviando…" : "Enviarme el enlace →"}</button>
          </>
        )}
      </div>
    </div>
...
function ExhaustedModal({ loggedIn, onSubscribe, onLogin, onByok, onClose }) {
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div style={{ fontSize: 32, marginBottom: 16 }}>🚀</div>
        ...
        <div style={{ padding: 24, background: "var(--surface2)", borderRadius: 12, border: "1px solid var(--gold)", marginBottom: 16 }}>
          ...
          {loggedIn
            ...
            : <button className="btn btn-gold btn-lg" style={{ width: "100%", marginTop: 14 }} onClick={onLogin}>Inicia sesión para suscribirte →</button>}
        </div>
...
function App() {
  ...
  const [session, setSession] = useState(() => { try { return localStorage.getItem(SESSION_STORAGE) || ""; } catch { return ""; } });
  ...
  const mode = subscribed ? "pro" : (apiKey ? "byok" : "free");
  ...
  const refreshMe = (sess) => {
    const s = sess !== undefined ? sess : session;
    return fetch("/api/me?session=" + encodeURIComponent(s || ""))
      .then(r => r.json())
      .then(d => { setEmailState(d.email || ""); setSubscribed(!!d.subscribed); setFreeRemaining(d.remaining); return d; })
      .catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    if (loginToken) {
      verifyLogin(loginToken)
        .then(d => { try { localStorage.setItem(SESSION_STORAGE, d.session); } catch {} setSession(d.session); setEmailState(d.email); refreshMe(d.session); setView("home"); })
        .catch(() => {})
  ...
  const handleSaveApiKey = key => {
    saveApiKey(key); setApiKey(key); setShowApiModal(false); setShowExhausted(false); setError(null);
  };

  const goToApp = () => { setView("home"); window.scrollTo(0, 0); };
  const logout = () => { try { localStorage.removeItem(SESSION_STORAGE); } catch {} setSession(""); setEmailState(""); setSubscribed(false); refreshMe(""); };
  ...
  }, [apiKey, subscribed, session]);

  const startExtraction = useCallback(async (pdfData, existingId) => {
    setExtracting(true); setError(null); setRetryMsg("");
    try {
      const text = await withRetry(
        () => (apiKey && !subscribed) ? byokExtract(pdfData.base64, apiKey || loadApiKey()) : freeExtract(pdfData.base64, session),
        m => setRetryMsg(m)
      );
  ...
  const startExam = useCallback(async () => {
    ...
    try {
      const result = await withRetry(
        () => (apiKey && !subscribed) ? byokGenerate(pdf.textContent, numQ, apiKey || loadApiKey()).then(qs => ({ questions: qs })) : freeGenerate(pdf.textContent, numQ, session),
        m => setRetryMsg(m)
      );
      setRetryMsg("");
    ...
    setLoading(false);
  }, [pdf, numQ, apiKey, subscribed, session]);
  ...
  const useSavedPdf = saved => {
    setPdf(saved);
    if (saved.textContent) setView("config");
  ...
  const headerProps = { view, fallos, email, subscribed, onLogo: () => setView("landing"), onPanel: goToApp, onFallos: () => setView("fallos"), onApiKey: () => setShowApiModal(true), onLogin: () => setShowLogin(true), onLogout: logout };
  const modals = (
    <>
      {showApiModal && <ApiKeyModal onSave={handleSaveApiKey} onClose={() => setShowApiModal(false)} />}
      {showLogin && <LoginModal onClose={() => setShowLogin(false)} />}
      {showExhausted && <ExhaustedModal loggedIn={!!email} onSubscribe={() => startCheckout(session)} onLogin={() => { setShowExhausted(false); setShowLogin(true); }} onByok={() => { setShowExhausted(false); setShowApiModal(true); }} onClose={() => setShowExhausted(false)} />}
    </>
  );
  ...
  if (view === "landing") return (
    <div className="examia-app">{orbs}{modals}<Header {...headerProps} /><Landing onStart={goToApp} onSubscribe={() => { if (email) startCheckout(session); else setShowLogin(true); }} /></div>
  );
  ...
  if (view === "config") return (
    <div className="examia-app">{orbs}{modals}<Header {...headerProps} />
      {!subscribed && !apiKey && freeRemaining !== null && <div className="free-banner">🎁 Te quedan <b style={{ margin: "0 4px" }}>{freeRemaining}</b> exámenes gratis</div>}
  ...
  return (
    <div className="examia-app">{orbs}{modals}<Header {...headerProps} />
      {!subscribed && !apiKey && freeRemaining !== null && <div className="free-banner">🎁 Te quedan <b style={{ margin: "0 4px" }}>{freeRemaining}</b> exámenes gratis{freeRemaining === 0 && <button className="nav-pill" style={{ marginLeft: 10 }} onClick={() => setShowExhausted(true)}>Ver planes</button>}</div>}
      {subscribed && <div className="free-banner" style={{ color: "var(--green)", background: "var(--green-dim)", borderColor: "rgba(74,222,128,0.2)" }}>✨ Plan Pro activo · exámenes ilimitados</div>}
      <div className="main">
